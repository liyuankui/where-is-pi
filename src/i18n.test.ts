...
describe("F7 双语 locale 完整性", () => {
  ...
  test("t() 切换生效；未知 key 回退不崩", () => {
    ...
    expect(t("app.title")).toBe("Build π Without π");
    expect(t("nonexistent.key" as MsgKey)).toBe("nonexistent.key"); // 不抛错
    setLang("zh", false);
    expect(t("app.title")).toBe("拼出一个 π");
  });
});
