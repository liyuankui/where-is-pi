...
const zh = {
  ...
  "why.s1.p1":
    "数学把实数分成两层楼。一楼是<b>代数数</b>：能成为某个整系数多项式方程根的数。3、1/2、√2、(√5−1)/2——整数、分数、根式全是。二楼是<b>超越数</b>：不属于任何整系数多项式的根。一楼很大（可数无穷个），二楼更大（不可数），但二楼住客大多「无名」，叫得出名字的几乎都在一楼。",
  "why.s1.p2":
    "一楼是个封闭俱乐部：任意两个代数数做加、减、乘、除（除数非零），结果仍是代数数——代数数构成一个<b>域</b>。根号套根号也出不了圈：√(1+√2) 照样是某个整系数方程的根。",
  "why.s1.p3":
    "而 π 住在二楼（Lindemann，1882）。于是定理宣判：两个代数数相加，和还是代数数，永远爬不上一楼之外的 π。这不是「很难找到」，是<b>被证明不存在</b>。",

  "why.s2.title": "二、两千年悬案落幕",
  "why.s2.p1":
    "<b>1761 年，Lambert</b> 证明 π 是无理数——π 连两个整数之比都不是。方法是把 tan(x) 展开成连分数：若 x 是非零有理数，则 tan(x) 必是无理数；而 tan(π/4)=1 是有理数，故 π/4 无理，π 无理。",
  "why.s2.p2":
    "<b>1873 年，Hermite</b> 证明 e 是超越数。<b>1882 年，Lindemann</b> 把这套武器推向更远（现称 Hermite–Lindemann 定理：非零代数数 α 的 e^α 必超越），证明 π 也是超越数：若 π 代数，则 iπ 代数非零，e^(iπ) 应为超越数——但欧拉公式说 e^(iπ)=−1，是代数数，矛盾。",
  "why.s2.p3":
    "超越性顺带终结了一桩两千年的悬案：<b>化圆为方</b>——只用尺规作一个面积等于给定圆的正方形。尺规能作出的长度只能是代数数（这一点 Wantzel 1837 年已证），而作正方形面积等于 πr² 需要作出长度 √π，π 既超越，√π 也超越——不可能。从古希腊到 1882 年，无数人为它耗尽笔墨，最后由一个「不可能定理」合上卷宗。",

  "why.s3.title": "三、伪装把戏的诚实拆解",
  "why.s3.p1":
    "切蛋糕的把戏值得诚实交代：A(c) = ∫₀ᶜ 4/(1+x²) dx 的精确值是 4·arctan(c)。定义里确实只有整数和平方，但 arctan 的值域由 π 标定——它就是 π 的化身。B 同理。所谓「两个看不见 π 的数」，灵魂深处仍然是 π。",
  "why.s3.p2":
    "莱布尼茨级数也一样：每个<b>有限</b>部分和都是有理数，与 π 无关；但那个「拆出来的另一半」是无穷尾巴，而无穷尾巴的和正是 π/4 的余项。伪装在于「有限看得见、无限藏着 π」。",
  "why.s3.p3":
    "所以这个玩具的真正惊喜不是「绕过了定理」——定理从未被绕过——而是：π 虽然在<b>代数</b>世界里无处容身，却在<b>分析</b>（积分、级数、极限）世界里无处不在。伪装的乐趣，正在于让你亲手摸到这条边界线。",

  "why.s4.title": "四、进阶彩蛋",
  "why.s4.p1":
    "<b>e 的平行伪装。</b>e = 1 + 1/1! + 1/2! + 1/3! + … 同样可以「切」：前 3 项 = 2.5，干净的有理数；尾巴 = e − 2.5 ≈ 0.21828…，一个超越数。任何可计算超越数都能玩这套把戏（见下方小演示）。",
  "why.s4.p2":
    "<b>e+π 与 e·π：悬而未决。</b>数学家强烈相信两者都是超越数，但至今连「是无理数」都没证明。已知的是：它们不可能<b>同时</b>是有理数——若 e+π 与 eπ 都有理，则 e、π 是同一个整系数二次方程 x²−(e+π)x+eπ=0 的根，都是代数数，矛盾。彻底解决它的一条路是著名的 Schanuel 猜想（1960s）。顺带：e^π（格尔丰德常数）的超越性<b>已经</b>被证明（Gelfond，1929）。",
  "why.s4.p3":
    "<b>高斯积分：e 与 π 相认。</b>∫₋∞^∞ e^(−x²) dx = √π。钟形曲线下面积竟然长出 π——这就是为什么正态分布的公式里处处是 π。e 与 π 在代数上形同陌路（猜想彼此代数无关），在分析学深处却是一家人。",
  "why.s4.p4":
    "<b>柴廷常数 Ω：连伪装都不行。</b>存在这样的超越数：没有算法能逐位算出它（不可计算数）。对它，别说「两个代数数相加」，连「写出一个逼近它的级数」都做不到。π 的可贵之处反衬出来：它超越，但<b>可计算</b>——所以才能被我们这样玩。",
  ...
  "why.e.sub": "e = 1 + 1/1! + 1/2! + 1/3! + …（高精度计算，前 3 项是有理数 2.5，尾巴是超越数）",
...
const en: Record<MsgKey, string> = {
  ...
  "why.s1.p1":
    "Mathematicians sort the real numbers into two floors. Ground floor: <b>algebraic numbers</b> — numbers that are roots of polynomial equations with integer coefficients. 3, 1/2, √2, (√5−1)/2 — integers, fractions, radicals all live here. Upper floor: <b>transcendental numbers</b> — numbers that satisfy no such equation. The ground floor is vast (countably many), the upper floor vaster still (uncountably many) — yet almost every number you can name lives on the ground floor.",
  "why.s1.p2":
    "The ground floor is a closed club: add, subtract, multiply, or divide any two algebraic numbers (division by nonzero), and the result is again algebraic — the algebraic numbers form a <b>field</b>. Even nested radicals stay inside: √(1+√2) is still the root of some integer-coefficient equation.",
  "why.s1.p3":
    "And π lives upstairs (Lindemann, 1882). Hence the verdict: the sum of two algebraic numbers is algebraic, and can never climb up to π. Not \"hard to find\" — <b>proven not to exist</b>.",

  "why.s2.title": "2. A Two-Thousand-Year Case, Closed",
  "why.s2.p1":
    "<b>In 1761, Lambert</b> proved π irrational — not even a ratio of two integers. His tool: expanding tan(x) as a continued fraction, which shows that a nonzero rational x makes tan(x) irrational; since tan(π/4) = 1 is rational, π/4 must be irrational, hence so is π.",
  "why.s2.p2":
    "<b>In 1873, Hermite</b> proved e transcendental. <b>In 1882, Lindemann</b> pushed the same machinery further (now the Hermite–Lindemann theorem: e^α is transcendental for every nonzero algebraic α) and proved π transcendental: if π were algebraic, then iπ would be nonzero algebraic, so e^(iπ) would have to be transcendental — but Euler's identity says e^(iπ) = −1, which is algebraic. Contradiction.",
  "why.s2.p3":
    "Transcendence also closed a two-thousand-year-old case: <b>squaring the circle</b> — constructing, with ruler and compass alone, a square whose area equals a given circle's. Ruler-and-compass can only construct algebraic lengths (shown by Wantzel in 1837); matching areas would require constructing √π, and if π is transcendental then so is √π — impossible. From ancient Greece to 1882, countless people poured ink into it, until an impossibility theorem finally closed the file.",

  "why.s3.title": "3. An Honest Confession about the Disguises",
  "why.s3.p1":
    "The cake trick deserves an honest confession: A(c) = ∫₀ᶜ 4/(1+x²) dx is exactly 4·arctan(c). The definition indeed contains only integers and a square — but arctan's very scale is set by π. It is π in disguise. Same for B.",
  "why.s3.p2":
    "The Leibniz series too: every <b>finite</b> partial sum is rational, unrelated to π; but the \"other half\" of the split is an infinite tail, and that tail sums to the remainder of π/4. The disguise works because the finite part is visible while π hides inside the infinite.",
  "why.s3.p3":
    "So the real surprise is not that we \"beat the theorem\" — the theorem was never beaten — but this: π has no home in the world of <b>algebra</b>, yet is everywhere in the world of <b>analysis</b> (integrals, series, limits). The fun of the disguise is letting you touch that boundary with your own hands.",

  "why.s4.title": "4. Going Deeper",
  "why.s4.p1":
    "<b>The parallel disguise for e.</b> e = 1 + 1/1! + 1/2! + 1/3! + … can be \"sliced\" the same way: the first 3 terms make 2.5, a clean rational; the tail, e − 2.5 ≈ 0.21828…, is transcendental. Every computable transcendental number can play this game (see the little demo below).",
  "why.s4.p2":
    "<b>e+π and e·π: still open.</b> Mathematicians strongly believe both are transcendental, yet no one has even proved them irrational. What is known: they cannot <b>both</b> be rational — if e+π and eπ were rational, then e and π would be the two roots of the integer-coefficient quadratic x²−(e+π)x+eπ=0, hence algebraic: contradiction. One road to a full answer is the famous Schanuel's conjecture (1960s). For the record: the transcendence of e^π (Gelfond's constant) <b>has</b> been proved (Gelfond, 1929).",
  "why.s4.p3":
    "<b>The Gaussian integral: e meets π.</b> ∫₋∞^∞ e^(−x²) dx = √π. The area under the bell curve grows a π — which is why π litters the formulas of the normal distribution. Algebraically, e and π are suspected strangers to each other (conjectured algebraically independent); deep down in analysis, they are family.",
  "why.s4.p4":
    "<b>Chaitin's constant Ω: no disguise possible.</b> There exist transcendental numbers that no algorithm can compute digit by digit (uncomputable numbers). For them, forget \"two algebraic numbers adding up\" — you cannot even write down a series that approaches one. Which makes π all the more precious: transcendental, yet <b>computable</b> — and that is exactly why we get to play with it like this.",
  ...
  "why.e.sub": "e = 1 + 1/1! + 1/2! + 1/3! + … (high-precision: the first 3 terms give the rational 2.5; the tail is transcendental)",
